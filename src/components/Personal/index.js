...
function Personal ({activeObjectId, setUserId, setPersonal, personal, dispatch}){
    ...
    const workers = personal.map((empolyee, i) => (
        empolyee.isFree &&
        <div 
            className={
                "personal__employee block__element " + 
                (empolyee.group === "М" ? "personal__employee_group1 " : "personal__employee_group2 ") +
                (empolyee.isGeodesist ? "personal__employee_geodesist" : "personal__employee_assistant")
            }
        ...
        >
            ...
            <div>
                <CloseBtn data={personal} setData={setPersonal} id={empolyee.id} name="personal" />
            </div>
        </div>
    ))
